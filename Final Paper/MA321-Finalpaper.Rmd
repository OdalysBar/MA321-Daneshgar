...
\begin{figure}
	...
	\caption{This contingency table shows the counts of how many articles are from each country. The countries shown above are only the ones from the data. All countries were not taken into account in this study.}
	\label{fig:CaffRun}
\end{figure}
...
\begin{figure}
	...
	\caption{The chi square test above assumes that each country has an equal probability to be represented in a journal article. The expected values are those shown in parantheses, and the observed values are above that. We hypothesized that the observed values would equal the expected values.}
	\label{fig:CaffRun}
\end{figure}
